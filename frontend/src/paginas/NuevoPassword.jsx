...
const NuevoPassword = () => {
  ...
  //Va a ejecutarse cuando el componente carge
  ...
  const handleSubmit = async e => {
    ...
    if(password.length < 6){
      setAlerta({ msg: "El password debe ser minímo de 6 caracteres", error: true});
      return;
    }
    try {
      const { data } = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/veterinarios/olvide-password/${token}`, { password });
  ...
  return (
    <>
      <div>
        <h1 className='text-indigo-600 font-black text-6xl'>Restablece tu passwordy no pierdas acceso{""}<span className='text-black'>tus pacientes</span></h1>
      </div>
        ...
        { tokenValido && (
          <>
            <form onSubmit={handleSubmit}>  
              <div className="my-5">
                <label className='uppercase text-gray-600 block text-lg font-bold' htmlFor="">Nuevo Password</label>
